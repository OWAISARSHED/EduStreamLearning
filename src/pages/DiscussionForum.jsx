...
import { MessageSquare, Eye, Clock, CheckCircle, Filter, Shield, UserCheck, BookOpen, X, Code, Send } from 'lucide-react';
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }} onClick={() => setShowModal(false)}>
          <div style={{ background: 'var(--bg-primary)', borderRadius: 12, padding: 24, width: 560, maxWidth: '90vw', maxHeight: '85vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
            ...
            <div className="form-group" style={{ marginBottom: 14 }}>
              ...
              <textarea style={{ width: '100%', padding: '8px 12px', borderRadius: 6, border: '1px solid var(--border-color)', background: 'var(--bg-secondary)', color: 'var(--text-primary)', fontSize: 13, fontFamily: 'monospace', minHeight: 80, resize: 'vertical' }} placeholder="Paste your code here..." value={newThread.code_snippet} onChange={e => setNewThread(p => ({ ...p, code_snippet: e.target.value }))} />
            </div>
            ...
            <button className="btn-primary" style={{ padding: '10px 20px', fontSize: 13, width: '100%' }} onClick={handleCreateThread} disabled={creating}>
